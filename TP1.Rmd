...
```{r}
empleo_argentina_mujeres <- filter(datos_sin_regiones,variable=="empleadoras_mujeres"|variable=="empleo_agricultura_mujeres"|variable=="empleo_industria_mujeres"|variable=="empleo_servicios_mujeres"|variable=="autoempleo_mujeres") %>% 
  pivot_longer(cols = -c(1:3)) %>% 
  rename(porcentaje=value) %>% 
  rename(anio=name) %>% 
  filter(anio==2018) %>% 
